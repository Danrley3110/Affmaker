      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <div className="mt-auto">
          <div className="flex items-baseline gap-2 mb-3">
            <span className="text-lg font-black text-gray-900">R$ {(product.promoPrice || 0).toFixed(2)}</span>
            {product.originalPrice > product.promoPrice && (
              <span className="text-xs text-gray-400 line-through">R$ {(product.originalPrice || 0).toFixed(2)}</span>
            )}
          </div>
...
export default function App() {
  ...
  const handleMLSearch = async (productId: string) => {
    const product = products.find(p => p.id === productId);
    if (!product || !product.mlProductId) {
      alert("Por favor, informe o ID do produto do Mercado Livre (ex: MLB3456789).");
      return;
    }
    
    const mlId = product.mlProductId.trim().toUpperCase();
    
    setIsExtracting(productId);
    try {
      // Busca dados básicos do produto
      const response = await fetch(`https://api.mercadolibre.com/items/${mlId}`);
      if (!response.ok) throw new Error("Erro ao buscar produto no Mercado Livre. Verifique se o ID está correto.");
      
      const data = await response.json();
      
      // Busca descrição (endpoint separado)
      const descResponse = await fetch(`https://api.mercadolibre.com/items/${mlId}/description`);
      let description = "";
      if (descResponse.ok) {
      ...
      setProducts(prev => prev.map(p => {
        if (p.id === productId) {
          return {
            ...
            promoPrice: promoPrice || p.promoPrice,
            originalPrice: originalPrice || p.originalPrice,
            discount: discount || p.discount,
            image: data.pictures?.[0]?.secure_url || data.pictures?.[0]?.url || p.image,
          };
        }
        return p;
      }));
      // Auto-open preview after successful search
      setPreviewProductId(productId);
    } catch (error) {
      console.error("ML Search error:", error);
      alert("Erro ao buscar dados do Mercado Livre. Verifique o ID do produto.");
    } finally {
  ...
  return (
    <div className="min-h-screen pb-20" style={{ '--primary': settings.primaryColor } as React.CSSProperties}>
                            ...
                            <div className="flex-grow space-y-1">
                              ...
                              <div className="flex flex-col gap-2">
                                <div className="relative flex items-center flex-grow">
                                  <input 
                                    className="w-full bg-transparent text-[10px] text-gray-500 outline-none border-b border-transparent focus:border-primary pr-16"
                                    value={p.affiliateLink}
                                    placeholder="Link do produto afiliado"
                                    onChange={(e) => {
                                ...
                                <div className="flex gap-2 items-center">
                                  <input 
                                    className="flex-grow bg-transparent text-[10px] text-gray-500 outline-none border-b border-transparent focus:border-primary"
                                    value={p.mlProductId || ''}
                                    placeholder="ID do produto ML (exemplo: MLB3456789)"
                                    onChange={(e) => {
                                      const val = e.target.value;
                                      setProducts(prev => prev.map(item => item.id === p.id ? { ...item, mlProductId: val } : item));
                                    }}
                                  />
                                  <button 
                                    onClick={() => handleMLSearch(p.id)}
                                    disabled={isExtracting === p.id}
                                    className="flex-shrink-0 px-3 py-1 bg-primary text-white rounded-lg text-[10px] font-bold hover:brightness-110 transition-all disabled:opacity-50 flex items-center gap-1"
                                  >
                                    {isExtracting === p.id ? (
                                      <><Loader2 size={10} className="animate-spin" /> Buscando...</>
                                    ) : (
                                      <><Search size={10} /> Buscar Produto</>
                                    )}
                                  </button>
                                </div>
                              </div>
      ...
      <AnimatePresence>
        {previewProductId && previewProduct && (
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            >
              <div className="bg-white rounded-3xl p-2 mb-4 flex items-center justify-between shadow-xl">
                <span className="text-[10px] font-black uppercase tracking-widest ml-3 text-gray-400">Preview em Tempo Real</span>
                <button 
                  onClick={() => setPreviewProductId(null)}
              ...
              <ProductCard product={previewProduct} onOfferClick={() => {}} />
              <p className="text-center text-white text-[10px] font-bold uppercase tracking-widest mt-4 opacity-70">
                Edite os campos no painel para ver as mudanças aqui
              </p>
            </motion.div>
          </motion.div>
        )}
